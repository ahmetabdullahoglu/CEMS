...
def verify_initialization() -> dict:
    ...
    try:
        with db_manager.get_session_context() as db:
            ...
            results["critical_entities"] = {
                "superuser_exists": bool(superuser),
                "superuser_email": superuser.email if superuser else None,
                "main_branch_exists": bool(main_branch),
                "main_branch_code": main_branch.branch_code if main_branch else None,
                "main_vault_exists": bool(main_vault),
                "main_vault_code": main_vault.vault_code if main_vault else None,
                "base_currency_exists": bool(base_currency),
                "base_currency_code": base_currency.code if base_currency else None
            }
            
            # Check relationships
            if superuser and main_branch:
                results["relationships"]["superuser_assigned_to_branch"] = superuser.branch_id == main_branch.id
                results["relationships"]["superuser_manages_branch"] = main_branch.branch_manager_id == superuser.id
            
            # Verify role assignments
            if superuser:
                user_roles = db.query(UserRoleAssoc).filter_by(user_id=superuser.id, is_active=True).count()
                results["relationships"]["superuser_has_roles"] = user_roles > 0
            ...
            # Check for warnings
            if results["counts"]["exchange_rates"] == 0:
                results["warnings"].append("No exchange rates found")
            if results["counts"]["branch_balances"] == 0:
                results["warnings"].append("No branch balances found")
            if results["counts"]["customers"] == 0:
                results["warnings"].append("No sample customers found")
...
if __name__ == "__main__":
    ...
    try:
        if args.reset:
            ...
            print(f"\n🔑 Critical Entities:")
            for entity, status in results['critical_entities'].items():
                print(f"  {entity}: {'✅' if status else '❌'}")
            
            print(f"\n🔗 Relationships:")
            for rel, status in results['relationships'].items():
                print(f"  {rel}: {'✅' if status else '❌'}")
            ...
            if not results.get('issues') and not results.get('warnings'):
                print(f"\n✅ All checks passed!")
